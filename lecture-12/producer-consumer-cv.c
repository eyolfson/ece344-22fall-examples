...
static pthread_cond_t has_empty;
static pthread_cond_t has_filled;
static uint32_t nfilled;

void initialize_cvs() {
    pthread_mutex_init(&cv_mutex, NULL);
    pthread_cond_init(&has_empty, NULL);
    pthread_cond_init(&has_filled, NULL);
    nfilled = 0;
}
...
void* producer(__attribute__((unused)) void* arg) {
    while (sem_trywait(&producer_remaining_sem) == 0) {
        ...
        while (nfilled == buffer_size) {
            pthread_cond_wait(&has_empty, &cv_mutex);
        }
        fill_slot();
        ++nfilled;
...
void* consumer(__attribute__((unused)) void* arg) {
    while (sem_trywait(&consumer_remaining_sem) == 0) {
        pthread_mutex_lock(&cv_mutex);
        while (nfilled == 0) {
            pthread_cond_wait(&has_filled, &cv_mutex);
        }
        empty_slot();
        --nfilled;
...
int main(int argc, char *argv[]) {
    ...
    /* Initialize condition variables used to ensure we don't overrun the buffer */
    initialize_cvs();
    ...
    pthread_cond_destroy(&has_filled);
    pthread_cond_destroy(&has_empty);
